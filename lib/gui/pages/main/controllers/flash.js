...
module.exports = function (
   ...
   * });
   */
  this.flashImageToDrives = (image, drives) => {
    ...
    return ImageWriterService.flash(image.path, drives).then((writerPromises) => {
      _.forEach(writerPromises, (promise, driveDevice) => {
        promise.then(() => {
          if (!flashState.wasLastFlashCancelled()) {
            notification.send('Success!', {
              body: messages.info.flashComplete({
                imageBasename: path.basename(image.path),
                drive: _.find(drives, [ 'device', driveDevice ])
              }),
              icon: iconPath
            })
          }
      ...
      return Bluebird.props(writerPromises).then(() => {
        console.log(`Was last flash cancelled? ${flashState.wasLastFlashCancelled()}`)
        if (!flashState.wasLastFlashCancelled()) {
          // XXX I (Shou) assume we don't want this because we want to see the states of
          // all the drives at the end
          //$state.go('success')
        }
      })
    }).catch((error) => {
  ...
  this.getProgressButtonLabel = () => {
    ...
    return progressStatus.fromFlashState(flashState.getAverageFlashState())
  }
}
