...
module.exports = function(
  ...
  DriveInfoModalService
) {

  /**
   * @summary Flash image to a drive, with a confirmation modal on system drives
   * @function
   * @public
   *
   ...
   * });
   */
  this.flashImageToDriveModal = (image, drive) => {
    if (drive.system) {
      DriveInfoModalService.display({
        confirmationLabel: 'Yes, flash',
        rejectionLabel: 'Change',
        icon: 'alert',
        title: 'ATTENTION',
        header: 'FLASH SYSTEM DRIVE?',
        description: 'All data on your system will be lost.'
      }).then((userAccepted) => {
        if (userAccepted) {
