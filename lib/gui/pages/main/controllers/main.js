...
module.exports = function (
  ...
  /**
   * @summary Display a tooltip with the selected drive details
   * @function
   * @public
   *
   * @returns {Promise}
   *
   * @example
   * MainController.showSelectedDrivesDetails()
   */
  this.showSelectedDrivesDetails = () => {
    analytics.logEvent('Show selected drives tooltip', {
      driveDevices: _.join(selectionState.getSelectedDevices(), ', ')
    })

    const driveDetails = _.join(_.map(selectionState.getSelectedDrives(), (drive) => {
      return `${drive.description} (${drive.displayName}) ${prettyBytes(drive.size)}`
    }), '\n')

    return TooltipModalService.show({
      title: 'Selected Drives',
      message: driveDetails
    }).catch(exceptionReporter.report)
  }

  /**
   * @summary Get the appropriate description label to display
   * @function
   * @public
   *
   * @returns {String}
   *
   * @example
   * console.log(MainContoller.getDescription())
   */
  this.getDescription = () => {
    const selectedDrives = selectionState.getSelectedDrives()
    if (selectedDrives.length > 1) {
      return `${selectedDrives.length} drives`
    }

    // middleEllipses errors on undefined, therefore fallback to empty string
    return _.get(selectionState.getCurrentDrive(), [ 'description' ]) || ''
  }

  /**
   * @summary Get the smallest of selected drive sizes
   * @function
   * @public
   *
   * @returns {Number}
   *
   * @example
   * console.log(MainContoller.getSmallestDriveSize())
   */
  this.getSmallestDriveSize = () => {
    return _.get(_.minBy(selectionState.getSelectedDrives(), 'size'), 'size')
  }

  /**
   * @summary Get the biggest of selected drive sizes
   * @function
   * @public
   *
   * @returns {Number}
   *
   * @example
   * console.log(MainContoller.getBiggestDriveSize())
   */
  this.getBiggestDriveSize = () => {
    return _.get(_.maxBy(selectionState.getSelectedDrives(), 'size'), 'size')
  }
}
