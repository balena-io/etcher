...
const storeReducer = (state = DEFAULT_STATE, action) => {
  switch (action.type) {
    ...
    case ACTIONS.SET_AVAILABLE_DRIVES: {
      ...
      if (numberOfDrives === AUTOSELECT_DRIVE_COUNT) {
        ...
        if (_.every([
          ...
          analytics.logEvent('Auto-Select Drive', {
            drive,
            image
          });
    ...
    case ACTIONS.REMOVE_DRIVE: {
      analytics.logEvent('Unselect drive', {
        device: state.getIn([ 'selection', 'drive' ])
      });
      return state.deleteIn([ 'selection', 'drive' ]);
    }

    case ACTIONS.REMOVE_IMAGE: {
      const image = state.getIn([ 'selection', 'image' ]) && state.getIn([ 'selection', 'image' ]).toJS();
      analytics.logEvent('Unselect image', {
        image
      });
