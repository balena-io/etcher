...
const storeReducer = (state, action) => {
  ...
  switch (action.type) {
    ...
    case ACTIONS.SET_AVAILABLE_CONNECTIONS: {

      if (!action.data) {
        throw new Error('Missing conneections');
      }

      if (!_.isArray(action.data) || !_.every(action.data, _.isPlainObject)) {
        throw new Error(`Invalid connection: ${action.data}`);
      }

      const newState = state.set('availableConnections', Immutable.fromJS(action.data));
    ...
    case ACTIONS.SELECT_NETWORK_TYPE: {

      return state.setIn([ 'selection', 'network_type' ], Immutable.fromJS(action.data));
    }

    case ACTIONS.SELECT_NETWORK_CONFIGURATION: {

      return state.setIn([ 'selection', 'network_configuration' ], Immutable.fromJS(action.data));
    }
    ...
    case ACTIONS.REMOVE_RELEASE: {
      return (state.deleteIn([ 'selection', 'download' ]) && state.deleteIn([ 'selection', 'network_type' ]) && state.deleteIn([ 'selection', 'network_configuration' ]));
    }
