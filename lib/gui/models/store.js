...
const storeReducer = (state, action) => {
  ...
  switch (action.type) {
    ...
    case ACTIONS.SET_AVAILABLE_DRIVES: {
      ...
      // Non-system drives
      const noSystemDrives = _.filter(action.data, (drive) => {
        return !drive.system;
      });

      const selectedDevice = newState.getIn([ 'selection', 'drive' ]);

      // There are non-system drives; set one as the selected drive.
      if (!selectedDevice && noSystemDrives.length >= 1) {

        const drive = _.first(noSystemDrives);
