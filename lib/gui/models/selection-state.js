...
const SelectionStateModel = angular.module(MODULE_NAME, [
  require('./drives'),
  require('./drive-constraints')
]);

SelectionStateModel.service('SelectionStateModel', function(DrivesModel, DriveConstraintsModel) {
   ...
   * - `DriveConstraintsModel.isDriveLocked()`
   *
   * @param {Object} drive - drive
  ...
  this.isDriveValid = (drive) => {
    return _.every([
      this.isDriveLargeEnough(drive),
      !DriveConstraintsModel.isDriveLocked(drive)
    ]);
  };
