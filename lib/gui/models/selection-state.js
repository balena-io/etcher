...
SelectionStateModel.service('SelectionStateModel', function() {
  ...
  this.getNetworkType = () => {
    return _.get(Store.getState().toJS(), 'selection.network_type');
  };
  this.getNetworkConfiguration = () => {
    return _.get(Store.getState().toJS(), 'selection.network_configuration');
  };
  ...
  this.getDownloadPath = () => {
    var download = _.get(Store.getState().toJS(), 'selection.download');
    if(download)
    {
      var boardMapping = [];
      ...
      var board = (boardMapping[download.name.split('-')[1]]) ? boardMapping[download.name.split('-')[1]] : download.name.split('-')[1];

      return download.tag_name + " - " + board;
    }
    return download;
  }
  ...
  this.hasNetworkType = () => {
    ...
    return Boolean(networkType && networkType.type && (networkType.type == 'eth' || (networkType.type == 'wifi' && networkType.connection)));
  }

  this.hasNetworkConfiguration = () => {
    var networkConfig = this.getNetworkConfiguration();
    return Boolean(networkConfig && networkConfig.configuration);
  };
  ...
  this.clear = (options) => {
    ...
    if (!options.preserveRelease) {

      Store.dispatch({
        type: Store.Actions.REMOVE_RELEASE
      });
