...
SelectionStateModel.service('SelectionStateModel', function(DrivesModel) {
  ...
  this.isDriveLargeEnough = (drive) => {
    const constraints = new Constraints(drive);
    return constraints.isLargeEnough(this.getImage());
  };
  ...
  this.isDriveSizeRecommended = (drive) => {
    const constraints = new Constraints(drive);
    return constraints.isRecommendedSize(this.getImage());
  };
  ...
  this.isDriveValid = (drive) => {
    const constraints = new Constraints(drive);
    return constraints.isValid(this.getImage());
  };
  ...
  this.isSystemDrive = (drive) => {
    const constraints = new Constraints(drive);
    return constraints.isSystemDrive();
  };
