...
class SafeWebview extends react.PureComponent {
  ...
  render() {
    if (this.state.shouldLoad) {
      ...
      return react.createElement('webview', {
        ...
        // A 'safe' local script that temporarily has Node access, injected into the webview.
        // See: https://electron.atom.io/docs/api/webview-tag/#preload
        preload: './components/safe-webview/preload/safe-webview',
        style: {
  ...
  componentDidMount() {
    ...
    if (this.state.shouldLoad) {
      ...
      this.refs.webview.addEventListener('will-navigate', this.willNavigate);
  ...
  componentWillUnmount() {
    ...
    if (this.state.shouldLoad) {
      ...
      this.refs.webview.removeEventListener('will-navigate', this.willNavigate);
  ...
  ipcMessage(event) {
    const maybeArg = _.get(event.args, 0);

    if (_.get(maybeArg, 'type') === 'error') {
      analytics.logException(_.get(maybeArg, 'message'));

    } else if (_.get(maybeArg, 'type') === 'event') {
      analytics.logEvent(_.get(maybeArg, 'message'));
    }
  }
