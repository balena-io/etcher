...
module.exports = function (
   ...
   * @example
   */
  this.driveClick = (drive, $event) => {
    let drives = []
    let selectionModifier = _.constant(selectionState.getSelectedDevices())

    if ($event.ctrlKey) {
      drives = [ drive ]
      selectionModifier = _.partial(_.forEach, _, selectionState.toggleDrive)
      ...
      selectionModifier = _.partial(_.forEach, _, selectionState.addDrive)
    } else {
      drives = [ drive ]
      selectionModifier = selectionState.setDrives
    }

    analytics.logEvent('Click drive', {
    ...
    return shouldChangeDriveSelectionState(drives).then((acceptedDrives) => {
      ...
      selectionModifier(_.map(acceptedDrives, 'device'))
    })
  }
