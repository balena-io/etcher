...
module.exports = function(
   ...
   * @param {Function} compare - function used to compare the old and new lists
   ...
   */
  const memoizeImmutableListReference = (func, compare = _.isEqual) => {
    let previous = [];

    return (...restArgs) => {
      const list = Reflect.apply(func, this, restArgs);

      if (!_.isEqualWith(list, previous, compare)) {
        previous = list;
      }
  ...
  /**
   * @summary Get the list of drives and their compatibility status objects
   * @function
   * @public
   *
   * @description
   * For each drive, return its compatibility status with the seleted image,
   * containing the status type (ERROR, WARNING), and accompanying
   * status message.
   *
   * @returns {Object[]} list of objects containing drive and statuses
   *
   * @example
   * const metadrives = DriveSelectorController.getDrivesAndStatuses();
   *
   * for ({ drive, statuses } of metadrives) {
   *   for ({ type, message } of statuses) {
   *     // do something
   *   }
   * }
   */
  this.getDrivesAndStatuses = memoizeImmutableListReference(() => {
    const statusList = _.map(this.drives.getDrives(), (drive) => {
      return {
        drive,
        statuses: this.constraints.getDriveImageCompatibilityStatuses(drive, this.state.getImage())
      };
    });
    ...
    return _.isEqual(
      utils.removeAngularHashes(newList),
      utils.removeAngularHashes(oldList)
    );
  });
