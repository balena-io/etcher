...
module.exports = function(
  ...
  const memoizeImmutableListReference = (func) => {
    ...
    return (...restArgs) => {
      let state = null;
      const newState = Reflect.apply(func, this, restArgs);

      previousTuples = _.map(previousTuples, ([ oldArgs, oldState ]) => {
        if (angular.equals(oldArgs, restArgs)) {

          // This argument was previously memoized, change 'state' from 'null'
          state = oldState;

          if (!angular.equals(newState, oldState)) {
            state = newState;
          }
      ...
      // 'null' state means the provided arguments were new and thus we push
      // them and their associated state to the tuple list for memoization
      if (state === null) {
        state = newState;
   ...
   *
   * @example
   * const statuses = DriveSelectorController.getDriveStatuses();
   *
   * for ({ type, message } of statuses) {
   *   // do something
   * }
   */
  this.getDriveStatuses = memoizeImmutableListReference((drive) => {
