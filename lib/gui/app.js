...
app.run(($timeout, WarningModalService) => {
  ...
  driveScanner.on('error', (error) => {
    if (error.message === 'LIBUSB_ERROR_ACCESS') {
      return permissions.isElevated().then((elevated) => {
        if (!elevated) {
          // Stop all adapters but notably usbboot, remove usbboot, and start the remaining adapters
          driveScanner.stop()
          driveScanner.adapters.delete('usbboot')
          driveScanner.start()

          WarningModalService.display({
            description: messages.warning.needRootForUsbboot({
