...
const app = angular.module('Etcher', [
  ...
  require('./modules/release-scanner'),
  require('./modules/image-writer'),
  require('./modules/wifi-scanner'),
  ...
  require('./components/kios-selector/kios-selector'),
...
app.controller('AppController', function(
  ...
  ReleaseScannerService.start();

  ReleaseScannerService.on('error', this.handleError);

  ReleaseScannerService.on('releases', (releases) => {
    this.kios.setReleases(releases);

    if (_.isEmpty(releases)) {
      ReleaseScannerService.close();
    }
  });
  ...
  WiFiScannerService.on('error', this.handleError);

  WiFiScannerService.on('connections', (connections) => {
    this.kios.setConnections(connections);

    if (_.isEmpty(connections)) {
      WiFiScannerService.close();
    }
  });
 ...
 this.openKiosSelector = () => {
    KiOSSelectorService.open()
      .then(this.selectVersion)
      .catch(this.handleError);
  };
  ...
  this.reselectKiOS = () => {
    ...
    this.openKiosSelector();
    AnalyticsService.logEvent('Reselect KiOS');
  };
