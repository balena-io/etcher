...
const githubReleases = require('../shared/github-releases');
...
app.run(() => {
  ...
  const includePreReleaseChannel = settings.get('includePreReleaseUpdateChannel');

  analytics.logEvent('Checking for updates', {
    currentVersion,
    updateSemverRange,
    includePreReleaseChannel
  });
  ...
  githubReleases.getLatestVersion({
    ...
    return updateNotifier.notify(latestVersion, {
      allowSleepUpdateCheck: release.getReleaseType(currentVersion) === release.RELEASE_TYPE.PRODUCTION
    });
