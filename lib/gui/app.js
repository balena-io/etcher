...
const app = angular.module('Etcher', [
  ...
  require('./components/file-dropper/file-dropper'),
...
app.controller('ApplicationController', function(
  ...
  /**
   * @summary List of supported file extensions for flashing.
   * @type {String[]}
   * @public
   *
   * @example
   * ApplicationController.supportedExtensions.join(', ');
   */
   ...
   *
   * @example
   * ApplicationController.dropImage({
  ...
  this.dropImage = (image) => {
    if (!SupportedFormatsModel.isSupportedImage(image.path)) {
      OSDialogService.showError('Invalid dropped image', `${image.path} is not a supported image type.`);
      AnalyticsService.logEvent('Invalid dropped image', image);
      return;
    }

    SelectionStateModel.setImage(image);

    AnalyticsService.logEvent('Dropped image', image);
  };

  /**
   * @summary Check if the current state page matches the argument.
   * @function
   * @public
   *
   ...
   *
   * @example
   * if (ApplicationController.isState('main')) doSomething();
   */
