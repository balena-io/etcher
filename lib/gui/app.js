...
const app = angular.module('Etcher', [
  ...
  require('./pages/progress/progress'),
...
app.run(() => {
  store.subscribe(() => {
    ...
    const averageFlashState = flashState.getAverageFlashState()
    ...
    analytics.logDebug(
      `Progress: ` +
      `${averageFlashState.percentage}% at ${averageFlashState.speed} MB/s ` +
      `(eta ${averageFlashState.eta}s)`
    )

    windowProgress.set(averageFlashState)
  })
})
