 ...
 * @param {Boolean} plain - with or without html additions
 *
 * @returns {String}
 *
 * @example
 ...
 * // '55% Flashing'
 */
exports.fromFlashState = (state, plain = false) => {
  ...
  if (state.percentage === utils.PERCENTAGE_MINIMUM && !state.speed) {
    ...
    if (state.percentage != null) {
      const txt = (plain) ? `${state.percentage}%` : `<strong>${state.percentage}%</strong>`
      return `Flashing... ${txt}`
    }
    return `${units.bytesToClosestUnit(state.position)} flashed`
  } else if (isValidating) {
    const txt = (plain) ? `${state.percentage}%` : `<strong>${state.percentage}%</strong>`
    return `Validating... ${txt}`
