...
analytics.service('AnalyticsService', function($log, $window, $mixpanel, SettingsModel) {
  ...
  this.logEvent = (message, data) => {
    ...
    // Map deeply over all non-object elements in an object.
    const deepMapValues = (f, value) => {
      if (typeof value === 'object') {
        return _.mapValues(value, (elem) => {
          return deepMapValues(f, elem);
        });
      }
    ...
    // Change 'long' strings to booleans in object `data`
    data = deepMapValues(truncateString, data);
