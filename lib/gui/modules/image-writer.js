...
imageWriter.service('ImageWriterService', function ($q, $rootScope, WarningModalService) {
  /**
  ...
  this.performWrite = (image, drive, onProgress) => {
    return $q((resolve, reject) => {
      ...
      child.on('timeout', (timeoutData) => {
        analytics.logEvent('Writer timeout', timeoutData)
        const timeoutDataSeconds = _.update(timeoutData, [ 'timeout' ], utils.millisecondsToSeconds)
        WarningModalService.display({
          confirmationLabel: 'OK',
          description: messages.error.writerTimeout(timeoutDataSeconds)
        })
      })
