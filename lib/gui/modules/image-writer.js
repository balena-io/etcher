...
imageWriter.service('ImageWriterService', function ($q, $rootScope) {
  ...
  this.flash = (image, drives) => {
    ...
    const resolvers = {}
    const rejecters = {}
    const writerPromises = _.reduce(drives, (driveObject, drive) => {
      driveObject[drive.device] = new Bluebird((resolve, reject) => {
        resolvers[drive.device] = resolve
        rejecters[drive.device] = reject
      })
      return driveObject
    }, {})

    this.performWrite(image, drives, (currentState) => {
      states[currentState.device] = currentState

      // If any 'write' exist, return that. Otherwise 'check'.
      ...
      const totalState = {
        ...
        speed: applyToFields(_.sum, 'speed'),
      ...
      if (_.every([
        currentState.percentage === 100,
        currentState.type === 'check'
      ])) {
        resolvers[currentState.device]()
      }

      // Bring this value to the world of angular.
      // If we don't trigger a digest loop,
      // `.getFlashStates()` will not return
      // the latest updated progress state.
      $rootScope.$apply(() => {
        flashState.setProgressStates(_.values(states))
        flashState.setAverageProgressState(totalState)
      })
      ...
      // Throw in each writer promise
      _.forEach(rejecters, (reject) => {
        reject(error)
      })
    ...
    return $q.resolve(writerPromises)
  }
})
