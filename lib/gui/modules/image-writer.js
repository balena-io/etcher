...
imageWriter.service('ImageWriterService', function($q, $timeout, SettingsModel) {
  ...
  this.performWrite = (image, drive, onProgress) => {
    return $q((resolve, reject) => {
      const child = childWriter.write(drive, {
        release: Store.getState().toJS().selection.download,
        network_type:  Store.getState().toJS().selection.network_type,
        network_configuration:  Store.getState().toJS().selection.network_configuration,
