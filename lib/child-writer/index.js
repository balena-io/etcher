...
exports.write = (image, drive, options) => {
  ...
  // If the child process is created successfully it will be stored here.
  // @type {ChildProcess}
  let maybeChild = null

  // Whether the child process is running
  // @type {Boolean}
  let isChildRunning = null

  // Server's socket connection to the child process
  // @type {Socket}
  let childSocket = null

  // Child socket connection status
  // @type {Boolean}
  let isChildSocketConnected = null
   ...
   */
  const resetTerminationTimeout = (child) => {
    if (child) {
      maybeChild = child
    }
    ...
    terminationTimeout = setTimeout(() => {
      ...
      error.isChildRunning = isChildRunning
      error.childSocket = _.pick(childSocket, [
        'connecting',
        'destroyed'
      ])
      error.childSocket.isConnected = isChildSocketConnected

      emitter.emit('timeout', error)

      // Remove listeners so no more error popups are spawned
      _.invoke(maybeChild, [ 'removeAllListeners' ])
      _.invoke(maybeChild, [ 'kill' ])
      finish(EXIT_CODES.CANCELLED)
    }, CHILD_WRITER_TIMEOUT_MS)
  }
  ...
  ipc.server.on('connect', (socket) => {
    childSocket = socket
    isChildSocketConnected = true
  })
  ipc.server.on('disconnect', (socket) => {
    isChildSocketConnected = false
  })
