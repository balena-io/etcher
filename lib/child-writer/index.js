...
exports.write = (image, drive, options) => {
   ...
   * @type {Timeout | undefined} [state.terminationTimeout]
   */
  const state = {}
  ...
  const finish = (code) => {
    terminateServer()
    clearTimeout(state.terminationTimeout)
  ...
  const resetTerminationTimeout = () => {
    clearTimeout(state.terminationTimeout)
    state.terminationTimeout = setTimeout(() => {
      ...
      _.invoke(state.child, [ 'removeAllListeners' ])
      _.invoke(state.child, [ 'kill' ])
