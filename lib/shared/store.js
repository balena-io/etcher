...
const DEFAULT_STATE = Immutable.fromJS({
  ...
  selection: {
    drives: new Immutable.OrderedSet()
  },
  isFlashing: false,
  flashResults: {},
  flashStates: [],
...
const storeReducer = (state = DEFAULT_STATE, action) => {
  switch (action.type) {
    case ACTIONS.SET_AVAILABLE_DRIVES: {
      ...
      const selectedDevices = newState.getIn([ 'selection', 'drives' ]).toJS()

      return _.reduce(selectedDevices, (accState, device) => {
        if (device && !_.find(action.data, {
          device
        })) {
    ...
    case ACTIONS.SET_AVERAGE_FLASH_STATE: {
      // TODO guard against invalid data
      return state.set('averageFlashState', Immutable.fromJS(action.data))
    }

    case ACTIONS.SET_FLASH_STATES: {
      ...
      const flashStates = _.filter(action.data, (flashState) => {
        if (!flashState.type) {
          return false
          throw errors.createError({
            title: 'Missing flash state type'
          })
        }

        if (!_.isString(flashState.type)) {
          return false
          throw errors.createError({
            title: `Invalid flash state type: ${flashState.type}`
          })
        }

        if (_.isNil(flashState.percentage)) {
          return false
          throw errors.createError({
            title: 'Missing flash state percentage'
          })
        }

        if (!utils.isValidPercentage(flashState.percentage)) {
          return false
          throw errors.createError({
            title: `Invalid flash state percentage: ${flashState.percentage}`
          })
        }

        if (_.isNil(flashState.eta)) {
          return false
          throw errors.createError({
            title: 'Missing flash state eta'
          })
        }

        if (!_.isNumber(flashState.eta)) {
          return false
          throw errors.createError({
            title: `Invalid flash state eta: ${flashState.eta}`
          })
        }

        if (_.isNil(flashState.speed)) {
          return false
          throw errors.createError({
            title: 'Missing flashState speed'
          })
        }
      ...
      return state.set('flashStates', Immutable.fromJS(flashStates))
    }

    case ACTIONS.RESET_FLASH_STATE: {
      return state
        .set('isFlashing', false)
        .set('flashStates', DEFAULT_STATE.get('flashStates'))
    ...
    case ACTIONS.SELECT_DRIVE: {
      ...
      const stateDrives = state.getIn([ 'selection', 'drives' ])

      return state.setIn([ 'selection', 'drives' ], stateDrives.add(action.data))
    }
    ...
    case ACTIONS.SELECT_IMAGE: {
      ...
      const selectedDevices = state.getIn([ 'selection', 'drives' ])

      return _.reduce(selectedDevices.toJS(), (accState, driveDevice) => {
        const drive = findDrive(state, driveDevice)
        if (drive && !constraints.isDriveValid(drive.toJS(), action.data)) {
          return storeReducer(accState, {
            type: ACTIONS.REMOVE_DRIVE,
            data: driveDevice
          })
    ...
    case ACTIONS.REMOVE_DRIVE: {
      ...
      const selectionDrives = state.getIn([ 'selection', 'drives' ])

      return state.setIn([ 'selection', 'drives' ], selectionDrives.delete(action.data))
    }
