...
exports.COMPATIBILITY_STATUS_MESSAGES = {
   ...
   *
   * @description
   * The image and drive compatibility is OK.
   */
  OK: '',
   ...
   * image's recommended drive size doesn't match the actual drive size.
   */
   ...
   *
   * @description
   * The drive is locked (SD cards) and cannot be written to.
   */
  LOCKED: 'Locked',
 ...
 *
 * @description
 * Status types classifying what kind of message it is, i.e. ok, error, warning.
 */
 ...
 * });
 *
 * const { type, message } = constraints.getDriveImageCompatibilityStatus(drive, image);
 *
 ...
 * }
 */
exports.getDriveImageCompatibilityStatus = (drive, image) => {
  ...
  if (!_.isNil(drive) && !exports.isDriveLargeEnough(drive, image)) {
    return {
      type: exports.COMPATIBILITY_STATUS_TYPES.ERROR,
  ...
  if (exports.isDriveLocked(drive)) {
    return {
      type: exports.COMPATIBILITY_STATUS_TYPES.ERROR,
  ...
  return {
    type: exports.COMPATIBILITY_STATUS_TYPES.OK,
    message: exports.COMPATIBILITY_STATUS_MESSAGES.OK
  };
};
