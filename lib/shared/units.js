...
/**
 * @summary Milliseconds in a second
 * @constant
 * @public
 * @type {Number}
 */
 ...
 * @public
 *
 * @param {Number} time - millisecond time
 * @returns {Number} seconds time fractional
 *
 * @example
 * const DELAY = 10000
 * setTimeout(() => {
 *   console.log(`${units.millisecondsToSeconds(DELAY)} seconds passed.`)
 * }, DELAY)
 */
exports.millisecondsToSeconds = (time) => {
  if (!Number.isFinite(time)) {
    throw errors.createError({
      title: `Not a finite number: ${time}`
    })
  }

  return time / exports.MILLISECONDS_IN_A_SECOND
}
