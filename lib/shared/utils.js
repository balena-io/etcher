...
/**
 * @summary Traverse an object and remove Angular hashes
 * @function
 * @public
 *
 ...
 *
 * @example
 * const angularObjA = { $$hashKey: 'abc' };
 * const angularObjB = { $$hashKey: '123' };
 *
 * const areObjectsEqual = _.isEqual(
 *   utils.removeAngularHashes(angularObjA),
 *   utils.removeAngularHashes(angularObjB)
 * );
 */
exports.removeAngularHashes = (object) => {
  if (_.isArray(object)) {
    return _.map(object, exports.removeAngularHashes);
  }
  if (_.isObject(object)) {
    const omitted = _.omit(object, [ '$$hashKey' ]);
    return _.mapValues(omitted, exports.removeAngularHashes);
  }
