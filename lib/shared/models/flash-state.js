...
/**
 * @summary Set the flashing states
 * @function
 * @public
 *
 * @param {Object[]} states - flashing states
 *
 * @example
 * flashState.setProgressStates([ {
 *   type: 'write',
 *   percentage: 50,
 *   eta: 15,
 *   speed: 100000000000
 * } ]);
 */
exports.setProgressStates = (states) => {
  const data = _.map(states, (state) => {
    const speed = _.attempt(() => {
    ...
    return Object.assign({}, state, { speed })
  })

  console.log(data)
  store.dispatch({
    type: store.Actions.SET_FLASH_STATES,
    data
  })
}

/**
 * @summary Set the average flashing state
 * @function
 * @private
 *
 * @description
 ...
 *
 * @example
 * flashState.setAverageProgressState({
 *   type: 'write',
 *   percentage: 50,
 *   eta: 15,
 *   speed: 100000000000
 * });
 */
exports.setAverageProgressState = (averageState) => {
  store.dispatch({
    type: store.Actions.SET_AVERAGE_FLASH_STATE,
    data: averageState
  })
}
...
/**
 * @summary Get the current flash states
 * @function
 * @public
 *
 * @returns {Object[]} flash states
 *
 * @example
 * const flashStates = flashState.getFlashStates();
 */
exports.getFlashStates = () => {
  return store.getState().get('flashStates').toJS()
}

exports.getAverageFlashState = () => {
  return store.getState().get('averageFlashState').toJS()
}
