...
/**
 * @summary Quote and escape a string
 * @function
 * @private
 *
 ...
 *
 * @example
 * const result = quoteAndEscapeString('foo');
 * console.log(result);
 *
 * // Windows
 * "foo"
 ...
 * 'foo'
 */
const quoteAndEscapeString = (string) => {
  if (os.platform() === 'win32') {
    return `"${string}"`;
  }

  return `'${_.replace(_.toString(string), /'/g, '\\\'')}'`;
};
...
exports.getEnvironmentCommandPrefix = (environment) => {
  ...
  const argv = _.flatMap(environment, (value, key) => {
    ...
    if (isWindows) {
      return [ 'set', quoteAndEscapeString(`${key}=${value}`), '&&' ];
    }

    return [ `${key}=${quoteAndEscapeString(value)}` ];
  });
...
/**
 * @summary Build the elevate command
 * @function
 * @private
 *
 * @param {String[]} command - command and arguments
 * @param {Object} [environment] - environment variables
 * @returns {String[]} elevate command
 *
 * @example
 * const command = permissions.buildElevateCommand([ 'foo', 'bar' ], {
 ...
 *
 * // Windows
 * [ 'cmd.exe', '/c', '"set "ELECTRON_RUN_AS_NODE=1" && call foo bar"' ]
 *
 * // Unix
 * [ 'env', 'ELECTRON_RUN_AS_NODE="1"', 'foo', 'bar' ]
 */
exports.buildElevateCommand = (command, environment = {}) => {
  const prefixedCommand = _.concat(
    exports.getEnvironmentCommandPrefix(environment),
    _.map(command, quoteAndEscapeString)
  );
  ...
  if (os.platform() === 'win32') {
    return [
      'cmd.exe',
      '/c',
      quoteAndEscapeString(_.join(prefixedCommand, ' '))
    ];
  }
...
exports.elevateCommand = (command, options) => {
  const elevateCommand = exports.buildElevateCommand(command, options.environment);

  if (os.platform() === 'win32') {
    const elevator = Bluebird.promisifyAll(bindings.load('elevator'));
    return elevator.elevateAsync(elevateCommand).then((results) => {
      return {
  ...
  return sudoPrompt.execAsync(_.join(elevateCommand, ''), {
