...
class StandardAdapter extends EventEmitter {
  ...
  scan (options = {}, callback) {
    ...
    drivelist.listAsync().map((drive) => {
      ...
      const isRemovable = _.isNil(drive.isRemovable) ? true : drive.isRemovable
      const isLocalSystem = drive.isSystem || !isRemovable
      return !drive.error && (options.includeSystemDrives || !isLocalSystem)
    }).map((drive) => {
      drive.displayName = drive.raw
      if (process.platform === 'win32' && drive.mountpoints.length) {
