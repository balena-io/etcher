 ...
 * })
 */
const startFileServer = (device, endpoint, options, step = DEFAULT_FILE_SERVER_STEP) => {
    ...
    .then((fileMessage) => {
      ...
      return Bluebird.try(() => {
        if (fileMessage.command === protocol.FILE_MESSAGE_COMMANDS.GET_FILE_SIZE) {
          ...
          return queryBlobFromCache(fileMessage.fileName, options.readFile).then((fileBuffer) => {
        ...
        return startFileServer(device, endpoint, options, step + STEP_INCREMENT)
      })
    })
}
...
exports.scan = (options) => {
  ...
  /* eslint-enable lodash/prefer-lodash-method */
    ...
    debug(`Opening device: ${device.device} (${idPair})`)
    device.open()
    ...
    const emitProgress = (percentage) => {
      debug(`%c[${device.device}] (${idPair}) -> ${Math.floor(percentage)}%%`, 'color:red;')
    }
    ...
    return Bluebird.try(() => {
      ...
      // auto-discovery.
      // eslint-disable-next-line no-magic-numbers
      const STEPS_TOTAL = device.deviceDescriptor.idProduct === USB_PRODUCT_ID_BCM2708_BOOT ? 29 : 38

      return startFileServer(device, endpoint, {
        readFile: options.readFile,
        progress: (step) => {
          emitProgress((step * (PERCENTAGE_TOTAL - PERCENTAGE_START) / STEPS_TOTAL) + PERCENTAGE_START)
        }
      }).tap(() => {
        // eslint-disable-next-line no-magic-numbers
        emitProgress(100)
