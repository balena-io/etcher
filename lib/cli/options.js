...
module.exports = yargs
  ...
  .example('$0 -d /dev/disk2 -d /dev/disk3 ubuntu.iso')
  ...
  .check((argv) => {
    // Force drive field to be an array for consistency
    if (_.isString(argv.drive)) {
      argv.drive = [ argv.drive ]
    }

    if (robot.isEnabled(process.env) && !argv.drive.length) {
      throw errors.createUserError({
        title: 'Missing drive(s)',
        description: 'You need to explicitly pass at least one drive when enabling robot mode'
      })
    }
