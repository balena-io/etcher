...
return network.collectAsync((error, data) => {
    ...
    var suffix = (parseInt(ip.split(".").pop()) + 1)%250;

    var netmask = ipmodule.subnet(ip, ipv4.netmask);
    var gateway = netmask.firstAddress;
    ip = ip.substr(0, ip.lastIndexOf('.')+1) + suffix + '/' + netmask.subnetMaskLength;
...
form.run([
  ...
  override: {
    ...
    var assets = answers.kios.assets;
    ...
    return new Bluebird((resolve, reject) => {
        ...
        .on('data', function(response)
        {
            ...
            var percentage = currentSize/totalSize;
    ...
    .then((stream) => {
        ...
        stream
        .pipe(replacestream("static_ip=\"\"", "static_ip=\"" + answers.static + "\""))
