...
permissions.isElevated().then((elevated) => {
  ...
  return drivelist.listAsync().then((drives) => {
    return Bluebird.all(_.map(answers.drive, (argDrive) => {
      ...
      if (!selectedDrive) {
        throw errors.createUserError({
          ...
          description: `We can't find ${argDrive} in your system. Did you unplug the drive?`
        })
      }
  ...
  console.log(results)
  return Bluebird.all(_.map(results, (result) => {
    return Bluebird.try(() => {
      if (robot.isEnabled(process.env)) {
        return robot.printMessage('done', {
          sourceChecksum: result.flash.sourceChecksum,
          drive: result.drive
        })
      }
      ...
      console.log(messages.info.flashComplete({
        drive: result.drive,
        imageBasename: path.basename(result.imagePath)
      }))

      if (result.flash.sourceChecksum) {
        console.log(`Checksum: ${result.flash.sourceChecksum}`)
      }
