...
/**
 * @summary Get image extension from archive name
 * @function
 * @public
 *
 ...
 *
 * @example
 * const extension = utils.getImageExtension('quirky-pi2-sd-8gb-xerus-8.1.img.xz');
 * console.log(extension);
 *
 * > '.img'
 */
exports.getImageExtension = (archiveName) => {
  const IMAGE_EXTENSION_INDEX = 1;
  const imageExtension = _.split(archiveName, '.').reverse()[IMAGE_EXTENSION_INDEX];

  if (_.isUndefined(imageExtension)) {
    return null;
  }
